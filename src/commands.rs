...
pub(crate) const COMMANDS: &[SlashCommand] = &[
    ...
    SlashCommand {
        name: "mode",
        category: Category::Mode,
        summary: "switch to a configured mode variant profile",
        usage: "/mode [local|cloud|default]",
        help: "Without an argument, prints the current agent mode. With an argument, switches to the configured profile tagged with that mode variant. Forge auto-creates `local` and `cloud` variants only for the built-in opencode backend.",
        tui: true,
        plain: true,
    },
    SlashCommand {
    ...
    SlashCommand {
        ...
        help: "Show, set, or clear the provider session id that forge sends via the active profile's continuation settings. Profiles without continuation support reject this command.",
        tui: true,
        plain: true,
    },
    SlashCommand {
    ...
    SlashCommand {
        name: "paste-image",
        category: Category::Session,
        summary: "save a clipboard screenshot and reference it in the prompt",
        usage: "/paste-image",
        help: "Imports a PNG image from the system clipboard into `.codex/external-agent-harness/attachments/`. In the TUI it inserts a prompt reference; in the plain REPL it prints the saved path.",
        tui: true,
        plain: true,
    },
];
